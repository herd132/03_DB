...
-- <컬럼 값 산술 연산>
-- 컬럼 값 : 테이블 내의 한 칸 (== 한 셀)에 작성된 값 (DATA)

-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 연봉 조회
SELECT EMP_NO, EMP_NAME, SALARY, SALARY*12 FROM EMPLOYEE;
 ...
 * 
 * */
SELECT SYSDATE - 1 "하루 전", SYSDATE 현재시간, SYSDATE + 1 내일
FROM DUAL;
...
-- EMPLOYEE 테이블에서
-- 이메일 '_' 앞이 EMAIL LIKE '___#_%' ESCAPE '#'; 이면서
-- 부서코드가 'D9' 또는 'D6' 이고
...
WHERE DEPT_CODE = ('D1', 'D6', 'D9');

-- NOT IN
SELECT EMP_ID,EMP_NAME, DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN('D1', 'D6', 'D9') -- 12명 (NULL 미포함)
OR DEPT_CODE IS NULL; -- 14명 (부서코드가 없는(NULL) 2명 포함)
