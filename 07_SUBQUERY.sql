...
-- 상관쿼리 (이태림이 D8 부서에서 가장 빠른입사/퇴사자 걸러짐 - D8 부서 아예 제외)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
JOIN JOB USING(JOB_CODE)
...
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
				 ...
				 OR (SUB.DEPT_CODE IS NULL AND MAIN.DEPT_CODE IS NULL)) -- 소속없음 까지
ORDER BY HIRE_DATE;
...
FROM (SELECT EMP_NAME, SALARY
  	  FROM EMPLOYEE
	  ORDER BY SALARY DESC) -- 해석 순위 1위 : FROM 절에서 전체 직원의 SALARY 내림차순 정렬
WHERE ROWNUM <= 5; -- 해석 순위 2위 : WHERE절에서 이제 가상컬럼의 1~5행까지만 조회
