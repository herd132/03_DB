...
CREATE USER kyc_sample IDENTIFIED BY 1234;


-- 2. 새 연결(접속) 추가
-- ORA-01045: 사용자 KYC_SAMPLE는 CREATE SESSION 권한을 가지고 있지 않음; 로그온이 거절되었습니다.
--> 접속 권한
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한,... TO 사용자명;
GRANT CREATE SESSION TO kyc_sample;
...
GRANT CREATE TABLE TO kyc_sample;

ALTER USER kyc_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- kh_kyc / kyc_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_kyc.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) sample 계정에 EMPLOYEE 테이블 조회 권한 부여
-- [객체 권한 부여 방법]
-- GRANT SELECT ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO kyc_sample;


-- 3. (sample) 다시 kh계정의 EMPLOYEE 조회
SELECT * FROM kh_kyc.EMPLOYEE;
--> 조회 잘 된다!
...
REVOKE SELECT ON EMPLOYEE FROM kyc_sample;

-- 5. (sample) 다시 kh 계정의 EMPLOYEE 조회
SELECT * FROM kh_kyc.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다